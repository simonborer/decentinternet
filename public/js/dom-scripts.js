...
(function () {
  window.onbeforeunload = function () {
    var patternsNav = document.getElementById('patternsNav');
    if (patternsNav) {
  ...
  window.addEventListener('DOMContentLoaded', function () {
    if (document.getElementById('patternsNav')) {
      if (window.location.href.indexOf('patterns/') !== -1) {
        document.getElementById('patternsNav').scrollTop = parseInt(localStorage.getItem('scrollPoint'));
      } else {
        document.getElementById('patternsNav').scrollTop = 0;
      }
    }
  })
}());
...
var localAlertClose = localStorage.getItem('alertClose'),
...
var hideAlert = function(){
  ...
  localStorage.setItem('alertClose', 'true');
};

if (localAlertClose === 'true') {
  hideAlert();
}
